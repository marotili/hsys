			...
			Just tsGroup -> do
				...
				mapM_ (\name -> do
						...
						origin <- lift $ group .: "origin"
                                                mTrim <- lift $ group .:? "trim"
                                                let trim = fromMaybe True mTrim
						Object boundaries <- lift $ group .: "boundaries"

						tileSets.at (tsHead^.tsName)._Just.
							tsGroups.at name L..= (Just $ Group name (head origin, origin!!1) Map.empty trim)
